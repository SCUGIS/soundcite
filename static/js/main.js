...
function validate_form() {
    ...
    $("#create-error").remove();
    validate_required($("#start_field"),"開始時間", msgs);
    validate_time_field($("#start_field"),"開始時間", msgs);
    validate_required($("#end_field"),"結束時間", msgs);
    validate_time_field($("#end_field"),"結束時間", msgs);
    validate_required($("#linktext"),"連結文本", msgs);
    var start = getTimeAsMillis("#start_field");
    var end = getTimeAsMillis("#end_field");
    if (start && end && (end < start)) {
        msgs.push("結束時間必須在開始時間之後")
